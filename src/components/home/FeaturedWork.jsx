          ...
          return (
            ...
            >
              <Link to={`/work/${project.id}`} className="block w-full h-full text-left">
                ...
                <div className="absolute top-3.5 left-3.5 sm:top-4 sm:left-4 md:top-5 md:left-5 z-10 bg-[#14090C]/90 backdrop-blur-md px-2.5 sm:px-3 py-1 border border-[#38161E] shadow-sm">
                ...
                <div className="absolute top-3.5 right-3.5 sm:top-4 sm:right-4 md:top-5 md:right-5 z-10 bg-[#1A0B10]/90 backdrop-blur-md w-8 h-8 md:w-9 md:h-9 flex items-center justify-center border border-[#38161E] text-[#EDE6DD] group-hover:bg-[#8B2635] group-hover:border-[#B3394B] group-hover:text-white transition-all duration-300 shadow-sm">
                ...
                {/* Bottom Content Area */}
                <div className="absolute bottom-4 left-4 right-4 sm:bottom-5 sm:left-5 sm:right-5 md:bottom-6 md:left-6 md:right-6 text-white z-10">
                  {/* Technology Subtitle */}
                  {project.techSubtitle && (
                    <div className="font-mono text-[9px] md:text-[10px] text-[#B3394B] font-semibold mb-1 tracking-wider uppercase truncate">
                      {project.techSubtitle}
                    </div>
                  )}

                  {/* Project Title */}
                  <h3 className="font-sans text-lg sm:text-2xl md:text-[28px] text-[#F3ECE2] group-hover:text-white font-bold tracking-tight group-hover:translate-x-0.5 transition-transform duration-300 leading-tight mb-1.5 sm:mb-2">
                    {project.title}
                  </h3>

                  {/* Short Description */}
                  {project.shortDescription && (
                    <p className="font-sans text-xs sm:text-sm text-[#A89D91] font-normal line-clamp-2 max-w-xl leading-relaxed mb-2.5 sm:mb-3">
                      {project.shortDescription}
                    </p>
                  )}

                  {/* Footer Meta Row: Metric on Left, Action Link on Right */}
                  <div className="flex items-center justify-between pt-2 border-t border-[#261016]/80 text-[10px] sm:text-[11px] font-mono">
                    <span className="text-[#C4B9AC] font-medium truncate mr-2">
                      {project.metricHighlight || (project.year ? `Year: ${project.year}` : "")}
                    </span>
                    <span className="text-[#B3394B] group-hover:text-[#FF4D64] font-semibold tracking-wider uppercase inline-flex items-center gap-1 transition-colors shrink-0">
